...
const db = new Database('/tmp/opsdash.db');
...
if (userCount === 0) {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    const reportsPerDay = 2 + Math.floor(Math.random() * 2);
...
function checkThresholdsAndAlert(report: any, supervisorName: string) {
  ...
  if (defectRate > defectThreshold) {
    ...
    if (emailEnabled) insertEmail.run('ceo@opsdash.com', `[OpsDash Alert] ${title}`, message);
  }
...
app.put('/api/reports/:id', (req, res) => {
  ...
  try {
    ...
    db.prepare('UPDATE reports SET date=?, shift=?, machine_id=?, production_count=?, defect_count=?, primary_defect_reason=?, operational_cost=?, budget=?, downtime_minutes=?, downtime_reason=?, notes=? WHERE id=?').run(date, shift, machine_id, production_count, defect_count, primary_defect_reason, operational_cost, budget, downtime_minutes, downtime_reason, notes, id);
    res.json({ success: true });
  } catch (e: any) {
    res.status(400).json({ error: e.message });
  }
...
app.post('/api/ai/insights', async (req, res) => {
  ...
  if (!apiKey || apiKey === 'MY_GEMINI_API_KEY') {
    ...
    return res.json({ insight: insights[Math.floor(Math.random() * insights.length)], simulated: true });
  }
  ...
  try {
    ...
    res.json({ insight: response.text || 'Unable to generate insight at this time.', simulated: false });
  } catch (e: any) {
